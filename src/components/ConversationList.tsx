...
export default function ConversationList({
  ...
  console.log("ConversationList state", state);
  if (state.isLoading) {
    return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="shrink-0 text-xs text-neutral-400">
                  {formatTime(state.conversationsById[id].updatedAt)}
                </span>
              </div>
            
              {state.conversationsById[id].unreadCount > 0 && (
                <span key={id} className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-blue-500 px-1.5 text-xs font-medium text-white">
                  {state.conversationsById[id].unreadCount}
                </span>
              )}
